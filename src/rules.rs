...
cat opening a file: refused ahead of all of it. Read tool, or a flag (-A -n -v -T -E); rides along only reading a pipe. One file piped into another program refused too, several files joined not. Under /proc /sys /dev not counted, so a cat of one passes. Overruled by waiver:cat-read-waiver.
"#;
